...
1. 이 목표를 달성하기 위해 필요한 구체적이고 유한한 단계들로 나눠라(1개~3개 정도).
   사용자가 "하나씩", "순서대로" 같은 말을 명시하지 않았더라도, 목표 안에 서로
   구별되는 대상/항목이 여러 개 나열되어 있고(예: 도시 세 곳, 회사 세 곳, 코인
   세 개, 문서 세 종류) 각 항목마다 상당한 분량의 답변이 필요하다면, 항목별로
   별도 단계로 나눠라(최대 3단계). 목표가 짧고 단순해서 한 번에 끝난다면 1개만.
   이 목록은 지금 한 번만 정하고 이후 절대 바뀌지 않는다.
2. 방금 나눈 단계 중 아직 안 끝난 "첫 번째" 단계에 대해서만 실제로 답하라. 여러
   항목으로 나눴다면, 이번 답변에는 그 항목 하나의 내용만 담고 나머지 항목은
   절대 포함시키지 마라(각자 다음 호출에서 따로 다룬다).
3. 이번 답변으로 위 각 단계가 충족됐는지 하나씩 판단하라.

- answer: 사용자에게 보여줄 실제 답변 (2번에서 정한 그 하나의 단계 내용만)
